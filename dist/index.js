...
export class AdaraSDK {
    ...
    requireSigner() {
        if (!this.signer)
            throw new Error("Signer required for write operations. Pass a signer in the constructor.");
        return this.signer;
    }
    // ─── Registration ──────────────────────────────────────────────────
    async register(name, bio) {
    ...
    async browseTasks(filters) {
        ...
        if (filters?.minCash)
            params.set("minCash", filters.minCash);
        if (filters?.maxCash)
            params.set("maxCash", filters.maxCash);
        if (filters?.ventureId)
            params.set("ventureId", filters.ventureId);
        if (filters?.sort)
    ...
    // ─── Task Lifecycle ────────────────────────────────────────────────
    async claimTask(ventureInstance, taskId, optionId = 0) {
    ...
    async startTask(ventureInstance, taskId) {
        const vi = new ethers.Contract(ventureInstance, ABI.vi, this.requireSigner());
        return vi.startTask(taskId);
    }
    async submitTask(ventureInstance, taskId, deliverable) {
        const vi = new ethers.Contract(ventureInstance, ABI.vi, this.requireSigner());
    ...
    async postAgentMetadata(agentId, name, bio) {
    ...
    async createVenture(name, opts) {
        ...
        const weightsHash = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["uint32", "uint32", "uint32", "uint32"], [2500, 2500, 2500, 2500]));
        ...
        const log = receipt.logs.find((l) => {
            try {
                return factory.interface.parseLog({ topics: l.topics, data: l.data })?.name === "VentureCreatedV2";
            }
            catch {
                return false;
            }
